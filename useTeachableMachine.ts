...
// - loads tensorflow.js and speech commands libraries
...
export function useTeachableMachine(
  ...
  // loads all required external scripts in sequence:
  // tensorflow.js is the core ml library for running neural networks
  // speech-commands is the tensorflow model for audio spectrogram processing
  ...
  const loadScripts = useCallback((): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      tfScript.onload = () => {
        ...
        // load speech commands library after tensorflow is ready
  ...
  const startListening = useCallback(async () => {
    ...
    try {
      ...
      // - runs inference on each audio frame
      // - calls our callback with prediction scores
      await modelRef.current.listen(
        (result) => {
          ...
          if (topPrediction && topPrediction.probability >= confidenceThreshold) {
            // don't repeat same detection within 2 seconds
